...
# When `EXACT` is not specified, this function with set the Zephyr build system
...
#             MAJOR:  Revision format is a single digit, `x`.
#             MAJOR.MINOR:  Revision format is two digits, separated by `.`, `x.y`.
#             MAJOR.MINOR.PATCH:  Revision format is three digits, separated by `.`, `x.y.z`.
#
...
function(board_check_revision)
  ...
  elseif(BOARD_REV_FORMAT MATCHES "MAJOR(\.MINOR(\.PATCH)?)?$")
    ...
    if(CMAKE_MATCH_1)
      set(revision_regex "${revision_regex}(\.[0-9]+)")
    endif()
  ...
  if(NOT (BOARD_REVISION MATCHES "^${revision_regex}$"))
    message(FATAL_ERROR "Invalid revision format used for `${BOARD_REVISION}`. \
            This board uses revision format: ${BOARD_REV_FORMAT}.")
  endif()
  ...
  if(BOARD_REV_EXACT OR NOT DEFINED ACTIVE_BOARD_REVISION)
    message(FATAL_ERROR "Board revision `${BOARD_REVISION}` not found. \
            Please specify a valid board revision.")
  endif()
